...
const listDoctorsForCustomers = async (requestData, res) => {
  try {
    ...
    let whereCondition = {
      [Op.and]: [{ doctor_name: { [Op.not]: null } }],
    };
        ...
        records.map(async (record) => {
          ...
          return {
            ...
            department_name: record.department
              ...
              : "",
            entity_name: record.doctorEntity
...
const getSingleEntityDetails = async (req, res) => {
  try {
    ...
    if (entityAddress) {
      ...
      districtName =
        district && district.districtName ? district.districtName : "";
    ...
    const departmentList = await adminServices.listDeptByClinic(
      { entityId },
      res,
      1
    );
...
const amountDetails = async (req, res) => {
  try {
    ...
    const doctorPhone = await decrypt(
      encryptedPhone,
      process.env.CRYPTO_SECRET
    );
    ...
    const amountDetails = {
      consultationCharge,
      bookingFee: parseInt(bookingFee.fee),
    };
    ...
    if (docEntitydata) {
      ...
      if (docPhone) {
        ...
        return handleResponse({
          ...
          data: { encryptedPhone: encryptedPhone },
        });
